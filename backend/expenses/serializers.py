...
class PotentialExpenseSerializer(serializers.ModelSerializer):
    ...
    expenseCard = serializers.PrimaryKeyRelatedField(source='expense_card', queryset=PotentialExpenseCard.objects.all())

    class Meta:
        model = PotentialExpense
        fields = ('id', 'account', 'expenseCard', 'name', 'amount', 'description', 'pros', 'cons')

class PotentialExpenseCardSerializer(serializers.ModelSerializer):
    account = serializers.PrimaryKeyRelatedField(read_only=True) 
    category = serializers.CharField(source='name')
    options = PotentialExpenseSerializer(many=True, read_only=True)
